...
sessions = np.array(np.meshgrid(np.array(FilesDF['Imaging'].unique()), np.array(FilesDF['Reinforcer'].unique()[1:]))).T.reshape(-1, 2)
...
for sessionType in sessions[:-1]: #do not consider VTA/SMOF
    ...
    for i, binnedCatheterVel in enumerate(FilesDF['median_binnedCatheterVel.npy']):
        if FilesDF['Imaging'][i] == sessionType[0] and FilesDF['Reinforcer'][i] == sessionType[1]:
            ...
            if np.nan not in currentSession:
                num_sessions += 1
                sessionTypeSignal = np.concatenate((sessionTypeSignal, currentSession), axis = 0)
            else:
                print('This session will not be considered, since it is only composed of nan values {}'.format(currentSession))
    sessionTypeSignal = np.reshape(sessionTypeSignal, (num_sessions+1, num_bins))
    print(sessionTypeSignal[1:]) #starts in one to eliminate the first array which is a np.zeros()
    Signal = np.round(np.mean(sessionTypeSignal[1:], axis=0), 2)
    print('Signal: {}'.format(Signal))
    StdError = np.round((np.std(sessionTypeSignal[1:], axis=0)/np.sqrt(num_sessions)),2)
    ...
    if sessionType[0] == 'SN':
        ...
        ax_VTA.plot(np.linspace(begg_second, end_second, len(Signal)), Signal, label='Intragastric {} Sessions (n={})'.format(sessionType[1], num_sessions), linewidth=0.9)
...
#save the figure
#fig.savefig('E:\\.shortcut-targets-by-id\\1un_-G2CqE1eg6sx5KdpwrQRyPBJ1REFA\\All_video_data_baseline\\MovementIGinjection_PerSession.png')
#plt.close(fig)    # close the figure window

#TURN THIS INTO A FUNCTION THAT CAN BE CALLED IN 'PlotResults.py'
...
num_sessions = 0
for i, binnedCatheterVel in enumerate(FilesDF['binnedCatheterVel.npy']):
    if FilesDF['Reinforcer'][i] == 'SUCRALOSE' and FilesDF['Imaging'][i] == 'SN':
        num_sessions += 1
        currentSession = np.load(binnedCatheterVel)
